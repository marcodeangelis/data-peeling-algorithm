...
def plot_peeling_nxd(x,a,b,p:list=None,figsize=None,aspect='auto',label='X',marker='s',markercolor='grey',boxcolor='blue2',grid=True,baseline_alpha=0.075):
    n,d = x.shape
    l = len(b)
    fuzzyall,p = peeling_to_structure(a,b,kind='scenario',beta=0.01)
    ...
    for i in range(d):
        for j in range(d):
            ...
            if i==j: #on diagonal
                _=plot_one_fuzzy_grad(fuzzyall[:,i,:],data=x[:,i],p=p,ax=ax,color=boxcolor,baseline_alpha=baseline_alpha)
